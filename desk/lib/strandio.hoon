...
++  fetch-json
  ...
  (pure:m u.json)
::
++  fetch-cord-with-token
  |=  [token=@t url=tape]
  =/  m  (strand ,cord)
  ^-  form:m
  =/  bearer=@t  (crip (weld "Bearer " (trip token)))
  =/  =request:http  :*
    %'GET'
    (crip url)
    `header-list:http`~[['Authorization' bearer]]
    ~
  ==
  ;<  ~                      bind:m  (send-request request)
  ;<  =client-response:iris  bind:m  take-client-response
...
++  fetch-json-with-token
  ...
  =/  m  (strand ,json)
  ^-  form:m
  ;<  =cord  bind:m  (fetch-cord-with-token [token url])
  =/  json=(unit json)  (de-json:html cord)
  ?~  json
    (strand-fail %json-parse-error ~)
  (pure:m u.json)
...
++  hiss-request
  ...
  =/  m  (strand ,(unit httr:eyre))
  ^-  form:m
  ;<  ~  bind:m  (send-request (hiss-to-request:html hiss))  take-maybe-sigh
::
